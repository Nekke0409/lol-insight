...
- 문서의 설명문, 제목, 표의 일반 용어는 원칙적으로 한글로 작성한다.
- API 이름, 코드 식별자, 명령어, 설정 키 등 정확성을 위해 영어 표기가 필요한 기술 용어는 원문을 유지한다.
...
## Swagger/OpenAPI 유지보수

클라이언트 API를 추가하거나 변경할 때는 Controller와 DTO뿐 아니라 관련 OpenAPI 설명, 예시, 응답 코드, 헤더와 명세 테스트를 함께 검토한다.

코드에서 자동 추론되는 mapping, 기본값, 검증 범위, 응답 타입은 문서 어노테이션에 불필요하게 중복하지 않는다. 특히 기본값과 검증 범위를 별도로 선언해 서로 달라지지 않게 한다.

분석 대상, 오류 처리, 외부 호출 비용, rate limit, 비동기 처리 방식처럼 자동 추론하기 어려운 의미가 변경되면 관련 설명을 직접 갱신한다.

API 변경 시 생성된 `/v3/api-docs`가 실제 API contract와 일치하는지 관련 명세 테스트를 추가하거나 수정한다.
